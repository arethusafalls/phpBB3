...
$sort_by_text	= array('c' => $user->lang['SORT_TOPIC_TIME'], 'a' => $user->lang['SORT_AUTHOR'], 't' => $user->lang['SORT_TIME'], 'f' => $user->lang['SORT_FORUM'], 'i' => $user->lang['SORT_TOPIC_TITLE'], 's' => $user->lang['SORT_POST_SUBJECT']);
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	$sort_by_sql = array('c' => 't.topic_time', 'a' => 'u.username_clean', 't' => (($show_results == 'posts') ? 'p.post_time' : 't.topic_last_post_time'), 'f' => 'f.forum_id', 'i' => 't.topic_title', 's' => (($show_results == 'posts') ? 'p.post_subject' : 't.topic_title'));
	...
	*/
	...
	if ($search_id)
	{
		// setting up default sort conditions based on UCP
		...
		switch ($search_id)
		{
			...
			case 'active_topics':
				...
				*/
				...
				$show_results = 'topics';
				//$sort_key = ((!empty($user->data['user_topic_sortby_type'])) ? $user->data['user_topic_sortby_type'] : 'c');
				//$sort_dir = ((!empty($user->data['user_topic_sortby_dir'])) ? $user->data['user_topic_sortby_dir'] : 'd');
				$sort_by_sql['t'] = 't.topic_last_post_time';
				$sort_by_sql['s'] = 't.topic_title';
				$sort_by_sql['v'] = 't.topic_views';
				$sort_by_sql['a'] = 't.topic_first_poster_name';
				$sort_by_sql['r'] = 't.topic_posts_approved';
				$sort_by_sql['c'] = 't.topic_time';
...
*/
	...
	if ($search_id)
	{
		...
		else if ($search_id == 'unreadposts')
		{
			...
			$id_ary = array_keys(get_unread_topics($user->data['user_id'], $sql_where, $sql_sort, $total_matches_limit + 1, 0, false));
		}
		else if ($search_id == 'unanswered')
		{
			// Only return up to $total_matches_limit+1 ids (the last one will be removed later)
			$id_ary = array_keys(get_unread_topics($user->data['user_id'], $sql_where, $sql_sort, $total_matches_limit + 1, 0, true));
		}
		else
		{
			$search_id = '';
	...
	$hilit = $keywords == '*' ? '*' : phpbb_clean_search_string($keywords);
