...
*/
function get_unread_topics($user_id = false, $sql_extra = '', $sql_sort = '', $sql_limit = 1001, $sql_limit_offset = 0, $sql_never_read = false)
{
	...
	if ($config['load_db_lastread'] && $user->data['is_registered'])
	{
		...
		if ($sql_never_read)
		{
			// need to exclude topics with your posts, AND include topics that you've never read (not in tt & ft tables)
			$sql_extra_where = "
				t.topic_poster <> " . $user->data['user_id'] . " AND ((tt.mark_time IS NULL AND ft.mark_time IS NOT NULL AND t.topic_time > ft.mark_time) OR (tt.mark_time IS NULL AND ft.mark_time IS NULL))
				$sql_extra
				$sql_sort";
		}
		else
...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum')
{
	...
	*/
	...
	$template->assign_vars(array(
		...
		'U_SEARCH_UNREAD_TOPICS'=> append_sid("{$phpbb_root_path}search.$phpEx", 'search_id=unread_topics'),
