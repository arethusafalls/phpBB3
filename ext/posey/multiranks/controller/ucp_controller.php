	...
	public function edit_user_ranks()
	{
		...
		//if($this->request->is_set_post('submit-user'))
		{
			...
			while ($row = $this->db->sql_fetchrow($result))
			{
				...
				// do not allow 'moderator', 'probationary', 'founder'
				if ((strcmp(strtolower($row['rank_title']), 'founder') != 0) &&
						(strcmp(strtolower($row['rank_title']), 'moderator') != 0))
				{
