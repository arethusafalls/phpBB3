...
class display extends base
{
	...
	public function display_topic_preview($row, $block)
	{
		...
		$preview_img = $this->get_first_image($row['first_post_text']);
		if ($preview_img == '') {
			$preview_img = $this->get_user_avatar_helper($row, 'fp');
		}
		else{
			$preview_img = '<img src ="'.$preview_img.'"/>';
		}
		...
		$block = array_merge($block, array(
			...
			'TOPIC_PREVIEW_FIRST_AVATAR'	=> $preview_img,
	...
	protected function get_user_avatar_helper($row, $poster)
	{
		...
		//return '<img src ="https://pbs.twimg.com/profile_images/411758450700349440/fldsxTBx.jpeg"/>';
		...
		return ($avatar) ?:
				'<img class="avatar" src="' . $this->root_path . 'styles/' . rawurlencode($this->user->style['style_path']) . '/theme/images/no_avatar.gif' . '" width="' . self::AVATAR_SIZE . '" height="' . self::AVATAR_SIZE . '" />';
	}
	...
	protected function get_first_image($text)
	{
		preg_match("#\[(img)(:[a-z0-9]+)?\](.*?)\[#", $text, $matches);
